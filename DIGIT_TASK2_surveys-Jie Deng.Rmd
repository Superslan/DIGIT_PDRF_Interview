...
ggplot(ldat, aes(Satisfaction))+ geom_density()+labs (x="satisfaction",y="distribution") + facet_grid (.~ Course)
#or
ggplot(ldat, aes(Satisfaction,fill=as.factor(Course)))+ geom_density()+labs (x="satisfaction",y="distribution") 
```
...
bdat <- ldat %>%
  ...
  bdat<-group_by(ldat, Course)%>% summarise(mean=mean(Satisfaction),
median=median(Satisfaction),
sd=sd(Satisfaction),
min=min(Satisfaction),
max=max(Satisfaction))

table1<-table(ldat$Course,dat$Satisfaction) #frequency table
table2<-prop.table(table1)# proportions

#row proportions
table3<-round(prop.table(table1,1),2)
#column proportions
table4 <-round(prop.table(table1,2),2)
#insert your code here.
...
```
df<- ggplot(data=ldat,aes(x=Satisfaction))
df + geom_bar(stat="count", aes(fill=Course))
